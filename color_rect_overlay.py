...
# Remove the IMPROVING.png ext_resource because we don't need it anymore
if 'IMPROVING.png' in content:
    lines = content.split('\n')
    lines = [l for l in lines if 'IMPROVING.png' not in l]
    content = '\n'.join(lines)
...
[node name="ScreenOverlay" type="ColorRect" parent="CanvasLayer/CRTMonitor/ComputerBG"]
layout_mode = 1
anchors_preset = 8
anchor_left = 0.5
anchor_top = 0.5
...
grow_horizontal = 2
grow_vertical = 2
color = Color(0.02, 0.0, 0.05, 0.95)
...
"""
    ...
    print("Fixed CRT overlay successfully with ColorRect")
else:
